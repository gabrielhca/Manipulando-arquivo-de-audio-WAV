...
// Reverte os dados de um arquivo WAV
void arquivoReverso(const char *entrada, const char *saida) {
    FILE *in = fopen(entrada, "rb");
    FILE *out = fopen(saida, "wb");
    if (!in || !out) {
        perror("Erro ao abrir arquivos");
    ...
    int bytesPorAmostra = header.BitsPerSample / 8;
    int numAmostras = header.SubChunk2Size / (header.NumChannels * bytesPorAmostra);
    short *amostras = malloc(header.SubChunk2Size);
    ...
    fread(amostras, header.SubChunk2Size, 1, in);

    // Reverte as amostras
    for (int i = numAmostras - 1; i >= 0; i--) {
        fwrite(&amostras[i], bytesPorAmostra, 1, out);
    }
...
int main() {
    ...
    do {
        ...
        switch (tarefa) {
            ...
            case 4: arquivoReverso("darkness-code.wav", "audio_reverso.wav"); break;
